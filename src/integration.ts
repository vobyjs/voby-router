import { $, Observable, useCleanup } from 'voby';
...
function intercept<T>(
  intercepted: Observable<T>,
  get?: (v: T) => T,
  set?: (v: T) => T
): [() => T, (v: T) => void] {
  return [
    get ? () => get(intercepted()) : () => intercepted(),
    set ? (v: T) => intercepted(set(v)) : intercepted,
  ];
}
...
export function createIntegration(
  ...
  let ignore = false;
  const wrap = (value: string | LocationChange) =>
    typeof value === 'string' ? { value } : value;
  const signal = intercept<LocationChange>(
    $(wrap(get()), { equals: (a, b) => a.value === b.value }),
    undefined,
    (next) => {
      !ignore && set(next);
      return next;
